...
- **ML 层**在留出集上精确率 100.0%；注入召回
  ...
  4.3% —— ML 层是中文检测的唯一有效来源（regex 层中文召回
  0.0%）。
...
| 引擎 | TP | FP | FN | TN | 精确率 | 召回率 | F1 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| regex | 11 | 15 | 456 | 543 | 42.3% | 2.4% | 4.5% |
...
| --- | ---: | ---: | ---: |
| regex | 14 µs | 28 µs | 70,961 |
| ml | 38 µs | 63 µs | 26,067 |
| fused | 55 µs | 80 µs | 18,335 |
...
7. ~~**层一兼容字符归一化**~~ — **已完成（v4 追加）**。签名层此前对全角 / 数学字母 / 圈号
   ...
   **只增不减**（原文本永远先扫，结构上不可能丢检测）。有界性由 NFKC 自身保证（幂等），
   ...
   实测：留出集 regex 精确率 50.0% → 57.1%、召回 3.2% → 4.3%，
